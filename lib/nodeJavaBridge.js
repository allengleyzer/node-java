...
const jdkPathSource = path.join(baseDir, './javabridge/jvm_dll_path.json');
const jvm_dll_path = require(jdkPathSource);

// append working directory for linux, since that is the location of jdk
if(platform == 'linux') {
  const fullPath = path.join(baseDir, './jdk', jvm_dll_path);
  process.env.PATH += path.delimiter + fullPath;
} else {
  process.env.PATH += path.delimiter + jvm_dll_path;
}

var binaryPath = null;
try {
  if(fs.statSync && fs.statSync(path.join(baseDir, "./javabridge/Debug/nodejavabridge_bindings.node")).isFile()) {
    binaryPath = path.resolve(path.join(baseDir, "./javabridge/Debug/nodejavabridge_bindings.node"));
...
if (!binaryPath) {
  binaryPath = path.resolve(path.join(baseDir, "./javabridge/Release/nodejavabridge_bindings.node"));
}
